...
export default function SquadDetails() {
  ...
  const tabsArray: Tab[] = ['squad', 'matches', 'achievement'];
    ...
    >
      ...
      <div className="sticky top-[var(--status-bar-height)] z-40 bg-white/95 dark:bg-[#050B18]/95 backdrop-blur-md border-b border-slate-100 dark:border-white/5 shadow-sm">
        <div className="max-w-4xl mx-auto px-4 py-3">
          ...
          {/* Improved Tabs with Underline */}
          <div className="flex gap-4 border-b border-slate-100 dark:border-white/5">
            {(['squad', 'matches', 'achievement'] as Tab[]).map((tab) => (
              <button
                key={tab}
                onClick={() => setActiveTab(tab)}
                className={`flex-1 pb-3 text-[11px] font-black uppercase tracking-widest transition-all relative ${activeTab === tab ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-400'}`}
              >
                {tab}
                {activeTab === tab && (
                  <motion.div
                    layoutId="squadTabUnderline"
      ...
      <div className="relative w-full bg-white dark:bg-[#050B18] overflow-hidden">
        <motion.div
          ...
          onDragEnd={(_, info) => {
            ...
            if (info.offset.x < -swipeThreshold || info.velocity.x < -velocityThreshold) {
              if (currentTabIndex < tabsArray.length - 1) setActiveTab(tabsArray[currentTabIndex + 1]);
            } else if (info.offset.x > swipeThreshold || info.velocity.x > velocityThreshold) {
              if (currentTabIndex > 0) setActiveTab(tabsArray[currentTabIndex - 1]);
            }
        ...
        >
          {(['squad', 'matches', 'achievement'] as Tab[]).map((tabId) => (
            <div key={tabId} className="w-full shrink-0 px-4 pt-6 min-h-[60vh]">
              {tabId === 'squad' && (
