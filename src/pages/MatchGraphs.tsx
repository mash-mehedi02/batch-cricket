...
export default function MatchGraphs({ compact = false }: { compact?: boolean }) {
  ...
  const yMax = useMemo(() => {
    ...
    const val = Math.ceil(maxOverRuns / step) * step
    return isNaN(val) || val <= 0 ? 12 : val
  }, [maxOverRuns])

  // Helper for safe SVG coordinates
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#060b16] text-slate-900 dark:text-white pb-12 font-inter">
      {/* Mini Scorecard Header */}
      {!compact && (
        <div className="bg-[#0f172a] text-white p-4 sm:p-6 shadow-xl sticky top-0 z-50 border-b border-white/5">
          <div className="max-w-5xl mx-auto flex items-center justify-between gap-4">
            <div className="flex-1 flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-6">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center font-black text-xs text-white">{firstName[0]}</div>
                <div>
                  <div className="text-sm font-black uppercase tracking-tight text-white">{formatShortTeamName(firstName)}</div>
              ...
              <div className="hidden sm:block text-slate-800 font-black text-xs">VS</div>
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center font-black text-xs text-white">{secondName[0]}</div>
                <div>
                  <div className="text-sm font-black uppercase tracking-tight text-white">{formatShortTeamName(secondName)}</div>
            ...
            <div className="bg-white/5 px-3 py-1.5 rounded-xl border border-white/10 text-[10px] font-black uppercase tracking-widest text-slate-500">
              Match Graphs
            </div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-[#0f172a] rounded-[2rem] border border-slate-200 dark:border-white/5 shadow-sm overflow-hidden p-6 sm:p-8">
              ...
              return (
                <svg width={svgWidth} height={svgHeight} className="overflow-visible">
                  ...
                  {[0, 0.25, 0.5, 0.75, 1].map((p) => (
                    <g key={p}>
                      <line x1={leftPad} y1={height * (1 - p)} x2={svgWidth} y2={height * (1 - p)} stroke="currentColor" className="text-slate-200 dark:text-[#1e293b]" strokeWidth="1" />
                      <text x={leftPad - 8} y={height * (1 - p) + 4} textAnchor="end" className="text-[10px] fill-slate-400 dark:fill-slate-500 font-bold">{Math.round(p * yMax)}</text>
                    </g>
                  ))}

                  {combinedByOver.map((o, i) => {
                    const xBase = leftPad + i * overWidth + overWidth / 2
                    const hA = safeNum((o.aRuns / yMax) * height)
                    const hB = safeNum((o.bRuns / yMax) * height)
          ...
          <div className="mt-10 flex flex-wrap gap-6 pt-6 border-t border-slate-100 dark:border-white/5">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-xs font-black uppercase text-slate-600 dark:text-slate-300 tracking-wider">{secondName}</span>
            </div>
            <div className="flex items-center gap-3 ml-auto">
              <div className="w-3 h-3 rounded-full border-2 border-slate-600" />
              <span className="text-xs font-bold text-slate-500 uppercase tracking-widest">Wicket</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-[#0f172a] rounded-[2rem] border border-slate-200 dark:border-white/5 shadow-sm overflow-hidden p-6 sm:p-8">
          ...
          <div className="overflow-x-auto no-scrollbar pb-4">
            {(() => {
              ...
              const getX = (over: number) => leftPad + safeNum(over) * overWidth
              const getY = (runs: number) => height - (safeNum(runs) / (maxTotalRuns || 1)) * height
              ...
              return (
                <svg width={svgWidth} height={svgHeight} className="overflow-visible">
                  ...
                  {[0, 0.25, 0.5, 0.75, 1].map((p) => (
                    <g key={p}>
                      <line x1={leftPad} y1={height * (1 - p)} x2={svgWidth} y2={height * (1 - p)} stroke="currentColor" className="text-slate-200 dark:text-[#1e293b]" strokeWidth="1" />
                      <text x={leftPad - 8} y={height * (1 - p) + 4} textAnchor="end" className="text-[10px] fill-slate-400 dark:fill-slate-500 font-bold">{Math.round(p * maxTotalRuns)}</text>
                    </g>
                  ))}
                  ...
                  {Array.from({ length: totalOversToShow + 1 }).map((_, i) => (
                    <text key={i} x={getX(i)} y={height + 20} textAnchor="middle" className="text-[10px] fill-slate-400 dark:fill-slate-500 font-black">{i}</text>
                  ))}
                </svg>
              )
            })()}
          ...
          <div className="mt-10 flex flex-wrap gap-6 pt-6 border-t border-slate-100 dark:border-white/5">
            ...
            <div className="flex items-center gap-3 ml-auto">
              <div className="w-3 h-3 rounded-full bg-red-500 border-2 border-[#0f172a] shadow-sm" />
              <span className="text-xs font-bold text-slate-500 uppercase tracking-widest">Wicket (Red Dot)</span>
            </div>
          </div>
        </div>
      </div>
